...
export const linkModule = timed('linkModule', (
  ...
  for (const module of modules) {
    ...
    const {hash, globals, symbols, visibles, declarations, modules} = table;
    ...
    if (declarations) for (const {flags, func} of declarations) if (func && (flags & RF.External)) {
      const {name} = func;
...
export const loadModulesInOrder = timed('loadModules', (
  ...
  while (queue.length) {
    ...
    const {table: {modules, declarations}} = module;
    ...
    // Recurse into links
    if (declarations) for (const {func, flags} of declarations) if (func && (flags & RF.External)) {
      const {name} = func;
